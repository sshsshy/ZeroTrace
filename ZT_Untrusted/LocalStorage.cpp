...
class Block {
  ...
  unsigned char* serialize() {
    unsigned char* serialized_block = (unsigned char*) malloc(data_size);
    unsigned char *ptr = serialized_block;
...
unsigned char* LocalStorage::downloadBucket(uint32_t bucket_id, unsigned char* bucket, uint32_t bucket_size , unsigned char *hash, uint32_t hash_size, uint8_t level)
{
  /*
  //std::string fp = directoryFP + std::to_string(bucket_id);
  //unsigned char* data = (unsigned char*) malloc(data_size * Z);
  ...
  try {
    //printf("Name: %s, Pos : %d\n", file_name_this.c_str(),(bucket_id-1)*Z*size_for_level);
    std::ifstream file(file_name_this.c_str(),std::ios::binary);
    file.seekg((bucket_id-1)*(Z*size_for_level));
